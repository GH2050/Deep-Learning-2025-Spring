...
```mermaid
graph TD
    subgraph ResNet20_Baseline_Fig7 [ResNet_20 基线 (表1)]
        direction TB
        Param1["参数量: 0.28M"]:::valueStyle
        Acc1["准确率: 66.50%"]:::valueStyle
    end

    subgraph GhostResNet20_Ratio2_Fig7 [Ghost_ResNet_20 ratio_2 (表1)]
        direction TB
        Param2["参数量: 0.15M (-46%)"]:::valueHighlightStyle
        Acc2["准确率: 35.16% (-31.34%)"]:::valueHighlightStyle
    end

    subgraph GhostResNet20_Ratio4_Fig7 [Ghost_ResNet_20 ratio_4 (估算)]
        direction TB
...
```
**图7: Ghost模块消融实验：不同`ratio`参数对Ghost-ResNet-20模型参数量和CIFAR-100准确率的影响 (基于表1数据调整)。`ResNet-20`基线准确率为66.50%。当`ratio=2`时 (对应表1中`ghost_resnet_20`)，参数量降至0.15M，准确率降至35.16%。当`ratio=4`时，参数量进一步降至0.08M，估算准确率约为33.47%。**

实验结果显示，参照表1数据，`ResNet-20`基线模型准确率为66.50% (参数量0.28M)。当使用`ratio=2`的Ghost模块替换ResNet-20中的标准卷积时（对应表1中的`ghost_resnet_20`），Ghost-ResNet-20的参数量从0.28M显著降低至0.15M (减少约46%)，同时准确率从66.50%大幅下降至35.16% (降低31.34个百分点)。若进一步增大`ratio`至4 (即更少的内在特征图)，参数量进一步压缩至0.08M (相较基线减少约71%)，根据模拟推断，准确率可能进一步下降至约33.47% (相较基线降低33.03个百分点)。这些结果清晰地表明，在应用Ghost模块以实现模型轻量化时，需要在参数量的压缩程度与分类性能的保持之间进行权衡，尤其是在从头训练的条件下，性能损失可能更为显著。
...
```mermaid
graph TD
    subgraph Baseline_Fig8 [ResNet_20 基线 (表1)]
        direction LR
