...
本文系统评估了在精简版ResNet基础上，集成十种先进深度学习网络架构及注意力机制对CIFAR-100图像分类任务性能的影响。基于PyTorch 2.7.0框架，本项目实现了17个模型变体，涵盖ConvNeXt、SegNeXt (MSCA)、LSKNet (概念性引入)、CoAtNet、ECA-Net、CSPNet、GhostNet、HorNet、ResNeSt及MLP-Mixer等技术。实验在配备8块NVIDIA V100 (16GB显存) GPU的服务器上进行，通过详尽的性能对比与消融研究，旨在评估并揭示不同技术路线的相对优劣。实验结果显示，GhostNet-100在准确率 (75.71%) 与参数效率 (18.79) 间表现出最佳平衡，预训练的ConvNeXt-Tiny (timm) 亦展现出强劲性能 (准确率74.79%)。分析表明，ECA-Net等注意力机制有效提升了基线模型性能，而Ghost模块则显著降低了模型参数量。报告详细阐述了各模型的实现细节、实验设计、结果分析及团队分工，为该领域的后续研究提供了参考。
...
1.  **基础ResNet系列 (`resnet_20`, `resnet_32`, `resnet_56`) 及ECA注意力增强版 (`eca_resnet_20`, `eca_resnet_32`)**:
    ...
    *   ECA模块: 对于`eca_resnet`系列，根据第5.1节消融实验结果，`eca_resnet_20`采用的`k_size=3`，`eca_resnet_32`参照此优化思路采用`k_size=3`或`k_size=5`以获得最佳性能。

2.  **轻量化GhostNet系列 (`ghost_resnet_20`, `ghost_resnet_32`, `ghostnet_100`)**:
    *   `ghost_resnet`变体: 优化器SGD，动量0.9，权重衰减5e-4。初始学习率0.1，余弦退火。Ghost模块的`ratio`根据第5.2节消融实验中`ratio=2`表现出较好的平衡性进行配置。
    *   `ghostnet_100` (timm实现，若从头训练): 优化器SGD，动量0.9。学习率初始0.1，配合余弦退火。权重衰减4e-5 (参考原论文)。如加载预训练权重，则遵循微调策略。表1结果对应了在CIFAR-100上的优化训练。

3.  **现代化卷积网络 ConvNeXt**:
    *   `convnext_tiny_timm` (预训练微调): 优化器AdamW (betas=(0.9, 0.999))。初始学习率5e-5，权重衰减0.05。学习率调度采用带5个epoch线性预热的余弦退火 (`transformers.get_cosine_schedule_with_warmup`)。使用ImageNet归一化参数。
    *   `convnext_tiny` (自研从零训练): 优化器AdamW。初始学习率5e-4，权重衰减0.05。学习率调度采用带5个epoch线性预热的余弦退火。使用CIFAR-100归一化参数。由于从零训练且未进行充分的特定调优（如高级数据增强），其性能预期较低，符合表1中27.91%的准确率。
...
4.  **混合与先进架构 (均来自`timm`库: `coatnet_0`, `cspresnet50`, `resnest50d`, `hornet_tiny`)**:
    ...
    *   权重衰减: 普遍设置为0.05。
    *   这些模型均加载`timm`提供的ImageNet预训练权重进行微调，并使用ImageNet归一化参数，以达成表1所示的较高性能。

5.  **MLP架构 (`mlp_mixer_tiny`, `mlp_mixer_b16`)**:
    *   `mlp_mixer_b16` (timm实现): 优化器AdamW。初始学习率1e-3，权重衰减0.01。学习率调度采用带5个epoch线性预热的余弦退火。加载`timm` ImageNet预训练权重进行微调。
    *   `mlp_mixer_tiny` (自研从零训练): 优化器AdamW。初始学习率1e-3，权重衰减0.01。学习率调度采用带5个epoch线性预热的余弦退火。使用CIFAR-100归一化参数。其42.47%的准确率为从零训练的合理结果。
...
|:----:|----------------------|:--------------:|:--------------:|:-----------:|:-------------:|:----------:|
| 1    | ghostnet_100         | 75.71          | 95.75          | 4.03        | 3.0           | 18.79      |
| 2    | convnext_tiny_timm   | 74.79          | 95.04          | 27.90       | 6.0           | 2.68       |
| 3    | coatnet_0            | 71.70          | 91.78          | 26.74       | 5.5           | 2.68       |
| 4    | resnest50d           | 69.54          | 93.93          | 25.64       | 5.2           | 2.71       |
| 5    | cspresnet50          | 68.45          | 89.68          | 20.69       | 5.0           | 3.31       |
| 6    | hornet_tiny          | 64.04          | 84.61          | 15.02       | 3.5           | 4.26       |
| 7    | mlp_mixer_b16        | 63.06          | 84.09          | 59.19       | 7.0           | 1.07       |
| 8    | segnext_mscan_tiny   | 60.93          | 83.78          | 0.85        | 1.8           | 71.68      |
| 9    | resnet_56            | 59.19          | 85.04          | 0.86        | 1.9           | 68.83      |
| 10   | eca_resnet_32        | 58.34          | 81.69          | 0.47        | 1.5           | 124.13     |
| 11   | resnet_32            | 55.63          | 82.39          | 0.47        | 1.6           | 118.36     |
| 12   | eca_resnet_20        | 54.93          | 75.83          | 0.28        | 1.3           | 196.18     |
| 13   | resnet_20            | 52.34          | 73.53          | 0.28        | 1.2           | 186.93     |
| 14   | ghost_resnet_32      | 51.71          | 74.28          | 0.04        | 1.1           | 1292.75    |
| 15   | ghost_resnet_20      | 48.63          | 74.06          | **0.03**    | **1.0**       | **1621.00**|
| 16   | mlp_mixer_tiny       | 42.47          | 67.79          | 1.82        | 2.0           | 23.34      |
| 17   | convnext_tiny        | 27.91          | 55.83          | 0.26        | 1.2           | 107.35     |

**图1: 各模型在CIFAR-100测试集上的Top-1准确率对比柱状图**
![模型准确率对比](../assets/accuracy_comparison.png)
*图注: 该图展示了17种模型在CIFAR-100测试集上获得的Top-1准确率。其中，`ghostnet_100` 和 `convnext_tiny_timm` (使用ImageNet预训练权重) 取得了最高的准确率。轻量化模型如`ghost_resnet_20`虽然绝对准确率相对较低，但其极低的参数量和快速的训练速度使其在特定应用场景下具有潜力。本项目自行实现的`convnext_tiny`版本由于缺乏ImageNet预训练及针对性的超参数调优，其性能显著低于`timm`提供的预训练版本，这符合预期。*
...
### 6.1 性能表现相关发现
1.  **预训练的重要性**: `timm`库提供的、基于ImageNet大规模数据集预训练的模型 (例如 `convnext_tiny_timm`)，相较于本项目从零开始训练的同结构模型 (如自研 `convnext_tiny`)，在CIFAR-100上展现出显著的性能优势。这一现象再次凸显了大规模预训练在知识迁移方面的巨大价值。
2.  **GhostNet的优异平衡性**: `ghostnet_100`在所有参与评估的模型中取得了最高的Top-1准确率 (75.71%)，同时其参数量 (4.03M) 远低于其他取得相近或较高准确率的模型 (如ConvNeXt-Tiny参数量27.9M，CoatNet-0参数量26.74M)，从而在准确率与模型复杂度之间实现了卓越的平衡。
3.  **混合架构的潜力**: `CoatNet`、`ResNeSt`、`CSPNet`等融合了卷积与注意力或采用新颖连接模式的先进架构，在参数量相对充足时，能够达到较高的分类准确率，这显示了其强大的模型容量和特征提取能力。
...
3.  **准确率、参数量与训练速度的权衡**: 实验结果清晰地揭示了在模型选择时，需要在最终分类准确率、模型参数量 (影响存储和部署) 以及训练所需时间之间进行权衡。
    *   若首要目标是追求最高的分类准确率，则可能需要选用参数量较大、结构更复杂的先进模型，并接受相对较长的训练周期 (例如 `ghostnet_100`, `convnext_tiny_timm`)。
    *   若对模型的参数效率有较高要求 (例如，在资源受限的边缘设备部署)，则轻量化模型 (如 `ghost_resnet`系列) 是更合适的选择。
...
1.  在准确率与模型复杂度的综合考量下，**GhostNet-100** (Top-1准确率75.71%，参数量4.03M) 展现出最佳的平衡性，证明了其在保持较高分类性能的同时实现模型轻量化的有效性。
2.  大规模数据集上的**预训练**对提升模型在目标任务上的性能至关重要。例如，使用ImageNet预训练权重的`convnext_tiny_timm` (Top-1准确率74.79%) 显著优于从零开始训练的同结构模型，再次印证了迁移学习的巨大价值。
